...
public class Recorder
{
    ...
    public static Recorder getInstanse(Boolean recordingCompressed)
    {
        Recorder result = null;

        if(recordingCompressed)
        {
            result = new Recorder(	false,
                    AudioSource.MIC,
                    sampleRates[3],
        ...
        else
        {
            ...
            do
            {
                result = new Recorder(	true,
                        AudioSource.MIC,
                        sampleRates[i],
                        AudioFormat.CHANNEL_CONFIGURATION_MONO,
                        AudioFormat.ENCODING_PCM_16BIT);

            } while((++i<sampleRates.length) & !(result.getState() == Recorder.State.INITIALIZING));
        }
        return result;
    }
    ...
    */
     ...
     *
     */
    public Recorder(boolean uncompressed, int audioSource, int sampleRate, int channelConfig, int audioFormat)
    {
        try
        {
    ...
    public void setOutputFile(String argPath)
    {
        ...
        catch (Exception e)
        {
            ...
            {
                Log.e(Recorder.class.getName(), "Unknown error occured while setting output path");
            }
            state = State.ERROR;
        }
    }
